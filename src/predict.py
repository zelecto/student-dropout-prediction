...
def load_model(path: str | Path | None = None) -> tuple:
    ...
    if isinstance(data, dict):
        ...
        modelo = data
        threshold = 0.5
    print(f"[MODELO] Cargado desde: {path} (threshold={threshold:.2f})")
...
def predecir_estudiante(
    ...
    if detallado:
        ...
        for _, row in resultados.iterrows():
            estado = "DESERTOR" if row["prediccion"] == 1 else "CONTINÚA"
            print(f"  {estado:12s} | riesgo: {row['probabilidad_riesgo']:.2%} | nivel: {row['nivel_riesgo']}")
